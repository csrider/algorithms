...
/** Determine the max values in a sliding window.
 * Utilizes a double-ended queue.
 * NOTE: This works, but is really slow for large data (need to optimize)
 * @param {number[]} nums
 * @param {number} k
 * @return {number[]}
 */
var maxSlidingWindow = function(nums, k) {
    const [MIN_NUMS_LENGTH, MAX_NUMS_LENGTH] = [1, Math.pow(10, 5)];

    // Input validation
    if (nums.length < MIN_NUMS_LENGTH || nums.length > MAX_NUMS_LENGTH)
        return [];
    if (k < MIN_NUMS_LENGTH || k > MAX_NUMS_LENGTH)
        return [];
